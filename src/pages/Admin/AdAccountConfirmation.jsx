...
const AdAccountConfirmation = () => {
  ...
  // Debug logging
  console.log("Account Confirmation Debug:", {
    user,
    isAdmin,
    data,
    isLoading,
  ...
  const getStatusBadge = (user) => {
    ...
    if (accountStatus === "pending") {
      statusText = "Pending";
      statusClass =
        "px-4 py-1 text-[12px] font-bold rounded-full bg-warning text-warning-content";
    } else if (accountStatus === "email_verification") {
      statusText = "Email Verification";
      statusClass =
        "px-4 py-1 text-[12px] font-bold rounded-full bg-gray-300 text-neutral";
    } else if (accountStatus === "approved") {
      statusText = "Approved";
      statusClass =
        "px-4 py-1 text-[12px] font-bold rounded-full bg-info text-info-content";
    } else if (accountStatus === "declined") {
      statusText = "Declined";
      statusClass =
        "px-4 py-1 text-[12px] font-bold rounded-full bg-error text-error-content";
    } else if (accountStatus === "active") {
      statusText = "Active";
      statusClass =
        "px-4 py-1 text-[12px] font-bold rounded-full bg-success text-success-content";
          ...
          <div className="text-sm text-gray-600">
            {paginatedRequests.length > 0
              ? `Showing page ${currentPage} of ${totalPages} (${
                  data?.data?.pagination?.totalItems || paginatedRequests.length
                } total records)`
              : `No record found`}
          </div>
          {totalPages > 1 && (
